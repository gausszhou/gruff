...
Gruff 是一个 Markdown 到 ANSI 终端渲染器，运行时除了 goldmark（解析器）、mattn/go-runewidth（Unicode 宽度）之外，没有其他依赖。
...
3. **ANSI 感知自动换行** — `wrapText`（文档级）和 `wrapCellLines`（单元格级）使用 `displayWidth`（`runewidth.StringWidth`）仅计算可见字符，保留前导空格。
